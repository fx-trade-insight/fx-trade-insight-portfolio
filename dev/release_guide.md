...
## 6. Google Drive の exe を更新（Note 購入者向け）

1. Google Drive を開く
2. 配布フォルダ内の古い exe ファイルを削除（またはフォルダ外に移動）
3. 新しい `FxTradeInsight-Setup-1.1.0.exe` をアップロード

> ファイルを「同名で上書き」するとファイル ID が変わらず SettingsPage のダウンロードリンクがそのまま使える。ただし zip で配布している場合は zip ごと置き換えること。
...
### BOOTH
1. BOOTH 管理画面を開く
2. 商品ページのダウンロードファイルを新しい exe に差し替える
3. 商品説明文のバージョン表記を更新する

### Note
1. Note の有料記事を開いて編集モードに入る
2. バージョン表記・変更内容の記載を更新する
3. 必要であれば Google Drive のリンクを更新する
...
[ ] Google Drive の exe を差し替え
[ ] BOOTH のダウンロードファイルを差し替え
[ ] Note 記事のバージョン表記を更新
[ ] （GAS 変更があれば）clasp push & 再デプロイ
```
